...
export default function handleHandContainerClick(event) {
  ...
  if (targetDataset.action === removeLastBet && ((activeHand && activeHand.id === targetDataset.id))) {
    handleRemoveLastBet(event.target, this.handCoordinator);
  }
 ...
 */
function handleRemoveLastBet(target, handCoordinator) {
 ...
 */
function handleHandSelection(bettingContainer, bettingSpotList) {
